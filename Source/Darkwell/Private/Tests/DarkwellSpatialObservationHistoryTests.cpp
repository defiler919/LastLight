...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FDarkwellCurrentRegionTransfer,
 "Darkwell.PropLab.MovingRules.SpatialHistory.CurrentRegionTransfer",Darkwell::SpatialObservationHistoryTests::Flags)
bool FDarkwellCurrentRegionTransfer::RunTest(const FString&)
{
 using namespace Darkwell::SpatialObservationHistoryTests;
 FDarkwellSpatialObservationHistory H; H.Initialize(TEXT("CurrentTransfer"));
 H.BeginCurrentObservation(FTransform::Identity,Bounds(0));
 TArray<float> Coverage; Coverage.Init(1.f,H.GetRecords()[0].SpatialMemory.GetCells().Num());
 H.AdvanceCurrent(.5f,Coverage); H.FreezeCurrentForHiddenMovement();
 const auto Sealed=H.GetRecords()[0]; const auto Lifetime=H.GetPreparationLifetime();
 H.BeginCurrentObservation(FTransform(FVector(40,0,0)),Bounds(40)); H.AdvanceCurrent(.5f,Coverage);
 const auto Before=H.GetRecords()[H.GetCurrentIndex()];
 for(int32 Pass=0;Pass<40;++Pass)
 {
  const auto Previous=H.GetRecords()[H.GetCurrentIndex()].Epoch;
  const uint32 Epoch=H.RekeyCurrentObservation();
  TestTrue(TEXT("Transfer epochs stay monotonic"),Epoch>Previous);
  TestEqual(TEXT("No synthetic history is generated by same-pose transfer"),H.GetRecords().Num(),2);
  const auto& Current=H.GetRecords()[H.GetCurrentIndex()];
  TestTrue(TEXT("Current pose is not re-observed or relocated"),Current.SnapshotTransform.Equals(Before.SnapshotTransform,0));
  TestTrue(TEXT("Current knowledge and blends are transferred exactly"),
   Current.SpatialMemory.GetCells().Num()==Before.SpatialMemory.GetCells().Num()
   && FMemory::Memcmp(Current.SpatialMemory.GetCells().GetData(),Before.SpatialMemory.GetCells().GetData(),Before.SpatialMemory.GetCells().Num()*sizeof(FDarkwellSpatialPropMemory::FCell))==0);
  TestEqual(TEXT("Sealed epoch is untouched"),H.GetRecords()[0].Epoch,Sealed.Epoch);
  TestTrue(TEXT("Sealed knowledge is untouched"),FMemory::Memcmp(H.GetRecords()[0].SpatialMemory.GetCells().GetData(),Sealed.SpatialMemory.GetCells().GetData(),Coverage.Num()*sizeof(FDarkwellSpatialPropMemory::FCell))==0);
  TestEqual(TEXT("Preparation lifetime is not reset"),H.GetPreparationLifetime(),Lifetime);
 }
 H.FreezeCurrentForHiddenMovement();
 TestEqual(TEXT("An actual exit still seals one historical pose"),H.GetRecords().Num(),2);
 TestEqual(TEXT("No Current cannot be transferred"),H.RekeyCurrentObservation(),uint32(0));
 return true;
}
