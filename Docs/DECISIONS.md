...
| Gray object policy candidate | Independent Reveal / MinimumObservedSpanCm / History fields, resolved once at registration. DARKWELL uses WholeObjectAfterSpan / 100 cm / StationaryOnly; no-config plugin compatibility stays SpatialPartial / 100 cm / Always. Local continuous-footprint confirmation never grants world visibility through walls. Candidate remains PARTIAL until performance, validation and user review close; see SIGHTWEAVE_GRAY_OBJECT_POLICY_HOME.md. |
| Gray history capacity and current | Retain at most 64 sealed histories plus one unsealed current reserve per object. A full history budget refuses capture with diagnostics but permits the next real legal current observation. No automatic eviction, oldest-record clearing or fabricated empty evidence; only fully VerifiedEmpty history may release. Superseded ownership alone cannot release a record. |
...
| Gray policy manual acceptance map | `/Game/Maps/L_SightWeaveGrayPolicyLab` is the dedicated project-layer Lab: central lobby, six spatially isolated Chinese-guided rooms, local reset/return and dormant-by-default stress fixtures. The legacy gameplay Lab remains for regression. |
| Gray history runtime broad phase | Retained history stays authoritative, but only unresolved tiles intersecting current view/sweep/geometry dirty regions become candidates; inactive records sleep without evidence work. Current ownership uses newly-owned dirty runs, an exact current-contributor path and a monotonic newer-epoch frontier. Force-full parity is retained; no StableID eviction, fabricated empty state, reduced quality or visible time-slicing is allowed. |
| Gray policy Lab V2 status | `PARTIAL — GRAY_POLICY_LAB_V2_READY_PERFORMANCE_BLOCKED`. The isolated map, Chinese UX and validation are ready, but 54,000+600 and ten-minute D3D12 performance remain outside the strict budgets. Black-layer work and a final gray stable ref require a later performance close plus explicit user acceptance. |
| Confirmed Whole current and history | Cone/range and physical-wall line of sight decide legal contact and confirmation. Once confirmed, any legal contact displays all registered primitives with one object blend, retaining ordinary camera depth; no per-part cone/wall presentation gate. Coherent eligible history is FullGeometryMask at LastLegalPose; Whole caps are forbidden. No world/neighbor discovery expansion. Unconfirmed Whole, SpatialPartial, StationaryOnly and Never retain their rules. See SIGHTWEAVE_CONFIRMED_WHOLE_CURRENT_HANDOFF_ZH.md; independent performance/exit blockers remain. |
| Whole qualification per continuous observation | Each new continuous legal session satisfies the same configured span. Seal eligible capture before EndSession; invalid coverage is not contact loss. Old memory and cached resources cannot grant qualification. Tentative Current only excludes overlapping historical draws reversibly, preserving durable memory and counterevidence. Qualified unchanged states may reuse recorded pose/content/resources. See SIGHTWEAVE_WHOLE_SESSION_HANDOFF_ZH.md. |
