...
> 2026-09-07 实现状态：P1完整可选路径已落地并推送9382461772468fd481a850783f2eba42a8706653，默认保持0；GT续算、两mask共享谓词、寿命/精确域、一次消费及同步回退已接通，未开始worker。功能/定向首显证据已通过，整体性能收益及默认启用门槛未通过；详见性能审计第23节及最新交接。以下伪接口保留为设计合同，实际内部组织见DarkwellHistoryPreparation与DarkwellWholePreparation；不能把此前“本轮仅文档”段落误读为最新实现状态。
