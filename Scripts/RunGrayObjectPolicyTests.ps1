...
$evidence = Join-Path $repo 'Saved/GrayObjectPolicy'
...
$begin = Get-Date
& "$EngineRoot/Engine/Binaries/Win64/UnrealEditor-Cmd.exe" "$repo/Darkwell.uproject" -unattended -nop4 -nosplash -NullRHI -NoSound "-ExecCmds=Automation RunTests $Tests" '-TestExit=Automation Test Queue Empty' "-ReportExportPath=$report" "-abslog=$log" *> (Join-Path $evidence "$RunName.stdout.txt")
$code = $LASTEXITCODE
