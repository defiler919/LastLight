...
| Confirmed Whole view-edge ownership | After confirmation, cone/range coverage decides only object-level legal contact. Current presentation is registered-primitive FullGeometryMask × uniform object blend × occlusion-only physical-wall gate; coherent Whole freeze is FullGeometryMask at LastLegalPose; Whole caps are forbidden. SpatialPartial keeps local observation/history/caps. Status remains PARTIAL pending user retest and the independent performance blocker. |
