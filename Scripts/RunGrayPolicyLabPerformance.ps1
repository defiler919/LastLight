...
$driver = Join-Path $repo 'Content/Python/profile_gray_policy_lab_v2.py'
...
$previousOutput = $env:DARKWELL_GRAY_POLICY_PERF_OUTPUT
$env:DARKWELL_GRAY_POLICY_PERF_OUTPUT = $evidence
try {
    $arguments = @(
    ...
    $env:DARKWELL_GRAY_POLICY_PERF_OUTPUT = $previousOutput
}
...
$summary = [ordered]@{
    ...
    severe_lines = @(Select-String -LiteralPath $log -Pattern 'Fatal error:|Assertion failed:|Ensure condition failed:|GPU crashed|EXCEPTION_ACCESS_VIOLATION|SightWeave activation failed|M6P1.*request failed|GRAY_POLICY_LAB_V2_PERF_FAIL').Count
    perf_pass = $content.Contains('GRAY_POLICY_LAB_V2_PERF_PASS ')
    pie_stopped = $content.Contains('GRAY_POLICY_LAB_V2_PERF_PIE_STOPPED')
    callback_unregistered = $content.Contains('GRAY_POLICY_LAB_V2_PERF_CALLBACK_UNREGISTERED')
