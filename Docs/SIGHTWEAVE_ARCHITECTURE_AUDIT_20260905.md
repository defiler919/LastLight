...
## Checkpoint H — release terminal residency; preserve newly acquired knowledge

- The production Current API now admits and seals actual new knowledge beyond 64 retained states. The old BeginObservedLocation compatibility entry retains its limit. Repeated same-state sessions still reuse one state; unresolved old positions are never evicted to satisfy a count. Memory can grow with genuinely new unresolved knowledge.
- Once a historical state has no residual effective surface and no residual 3D cap, its dense capture/geometry/evidence and remaining presentation cache can release after all candidate queries. The release predicate never converts Superseded into VerifiedEmpty, and does not reuse epoch numbers. Partially unresolved records cannot release. Existing fine/coarse compatibility diagnostics otherwise remain.
- New direct storage test proves partial knowledge cannot compact, fully replaced samples have no empty proof, terminal state can release, and deleted epochs cannot resume. Scene test duplicates an actually acquired capture solely as a residency fixture, then checks 4 -> 1 records while the newest valid surface stays visible. This fixture is not the independent visibility oracle.
- Failed evidence: Probe 1 crashed in the new test after it inserted records without publishing the geometry/ownership revisions used by the real host. A failed assertion was also followed by a mutating resume call. The fixture now publishes those revisions and returns on failed dependent prerequisites. Probe 2: 19 total, 18 passed, one assertion still demanded the new 65th record after it had legitimately been superseded by visible motion; the original 64 distant evidence hashes still matched. Updated that count to allow lawful compaction while prohibiting added moving history.
- `Audit_TerminalResidency_Contracts`: 110 tests, 107 pass (103 clean + 4 warnings), 3 failures, normal exit 0, severe 0, 127.908 s tests / 151.930 s wall. All three failures required a terminal record or its final text to remain allocated; per-sample parity itself passed. Migrated them to current visibility, no residual proxy/cap, and the captured pre-resolution evidence. `Audit_TerminalResidency_Migration`: all 5 selected tests pass, 4 clean + 1 warning, normal exit 0, severe 0, 30.531 s tests / 50.586 s wall, including independent-current capacity and idle work checks. Do not present this as a fresh full 110-test rerun.
- Formal builds: TerminalResidencyBuild 33.41 s, Build2/3/4 succeeded (final build log is `Saved/Logs/ArchitectureAudit_TerminalResidencyBuild4.log`). No Live Coding or binary asset edits.
- Two real Room02 debugger-attached captures at this stage (`Room02_DebugExit01/02`) each complete 51 frames and exit 0. Both independent surface oracles pass all six final samples, 896 interior probes each. Opened final, first full reentry and legal partial-cut original images: no final internal seams, newly unknown region dots, legal outer cut remains. Files are 2233x911, with the app's inspection display resized to 2048x836. These diagnostic runs are not performance acceptance.
- G's normal corrected protocol `Transitions_CurrentTiles` completed all 12 cases but **failed teardown**: exit 0xC0000005 after log close, 284.391 s wall. Partial system p95 31.615 -> 5.562 ms, 32 Whole 13.244 -> 8.411 ms (F -> G). Full frames still exceed 16.6 ms even in Empty. The batch first-update pathology remains: same-identity ownership proof ~6.25 s, distributed ownership proof ~17.26 s; this is not ordinary idle/view cost and is the next measured target.
- `WatchUnrealExit.cpp` is a Windows SDK diagnostic launcher that records access-violation/second-chance exception addresses, stack modules/symbols and minidumps through [DEBUG_EVENT](https://learn.microsoft.com/en-us/windows/win32/api/minwinbase/ns-minwinbase-debug_event) and [the documented debugger event loop](https://learn.microsoft.com/en-us/windows/win32/api/debugapi/nf-debugapi-waitfordebugevent). It has no runtime dependency and lives outside the game module. Build it with the installed MSVC `cl /std:c++17 /EHsc Scripts/WatchUnrealExit.cpp /Fe:Saved/ArchitectureAudit/WatchUnrealExit.exe /link dbghelp.lib psapi.lib`; outputs stay in Saved. Usage: output-directory, executable, executable arguments. Debugger timings are explicitly rejected as performance evidence. Normal application event logs returned no corresponding Unreal fault entry; two short attached runs did not reproduce the intermittent normal-exit fault. A full-protocol diagnostic attempt is ongoing, not a passed teardown claim.
- The H broad semantic run overlapped one graphical diagnostic run. Its two short timing-gate results are not accepted as isolated performance evidence; G's prior isolated pass remains the valid short-gate comparison. Repeat the strict gates in isolation on the final implementation.

## Checkpoint I — reuse batch ownership geometry

- Measured first-update ownership work collected the same newer primitive snapshots for each fine sample. The update now collects them once per historical record and rejects impossible footprint intersections with conservatively padded projected bounds. Exact center/corner/slab/contact predicates still decide ownership; no sample density, cap geometry, empty proof or material changed. Points with no old vertical interval no longer scan newer intervals.
- The full reference path bypasses the new snapshot/bounds shortcut. `BatchOwnershipSamplesEquivalent` compares state, empty verification, opacity, frozen envelope, dwell, coverage, occupancy, ownership and submitted visibility for every sample over a 16-pose batch and subsequent view changes. The analytic episode oracle remains separate from this implementation-parity test.
- Formal `Saved/Logs/ArchitectureAudit_BatchOwnershipBuild.log`: succeeded, 31.49 s. `Audit_BatchOwnership_Probe`: 2/2 selected tests, 25.168 s test / 45.351 s wall. `Audit_BatchOwnership_Contracts`: **111/111**, 104 clean + 7 warnings, normal exit 0, zero severe lines, 133.686 s tests / 154.143 s wall. No simultaneous GPU run or build.
