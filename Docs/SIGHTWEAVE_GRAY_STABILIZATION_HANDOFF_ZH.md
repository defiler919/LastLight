...
最新续工：从 `d986b53` 开始的性能架构审计正在进行，当前运行时已有 ownership/cap 局部重构。最新模型和实验见 [性能架构审计](SIGHTWEAVE_PERFORMANCE_ARCHITECTURE_AUDIT_ZH.md) 第 5 节；下文“最终”指上一轮已完成的稳定化基线，不能替代新运行时的阶段回归。
