...
## Current SightWeave manual entry

Run `Scripts/LaunchApartmentSightWeaveLab.ps1` to play `/Game/Maps/L_SightWeaveApartmentLab` (P4 + current DARKWELL adapter + ObjectMemoryScene). See [Apartment Lab](Docs/SIGHTWEAVE_APARTMENT_LAB_ZH.md).

**Legacy Deprecated:** `/Game/Maps/L_Prototype` is retained solely for the frozen historical performance Reference protocol. It is no longer a default or daily manual entry. Do not use it to validate current SightWeave. Historical replay requires explicit `-AllowLegacyReference`.

## Historical prototype status (Legacy Deprecated)
