...
$driver = Join-Path $repo 'Content/Python/profile_gray_policy_lab_v2_ten_minute.py'
$log = Join-Path $evidence "$RunName.log"
$trace = Join-Path $evidence "$RunName.utrace"
$begin = Get-Date
[ordered]@{
    head = (& git -C $repo rev-parse HEAD)
    started = $begin.ToString('o')
    fixed_timestep = $false
    required_wall_seconds = 600
...
$previousOutput = $env:DARKWELL_GRAY_POLICY_TEN_MIN_OUTPUT
$env:DARKWELL_GRAY_POLICY_TEN_MIN_OUTPUT = $evidence
try {
    $arguments = @(
        "$repo/Darkwell.uproject", '/Game/Maps/L_SightWeaveGrayPolicyLab',
        '-d3d12', '-sm6', '-nosound', '-unattended',
        '-trace=cpu,frame,bookmark,log', "-tracefile=$trace",
    ...
    $env:DARKWELL_GRAY_POLICY_TEN_MIN_OUTPUT = $previousOutput
}
...
$summary = [ordered]@{
    ...
    measured_wall_seconds = 0
    severe_lines = @(Select-String -LiteralPath $log -Pattern 'Fatal error:|Assertion failed:|Ensure condition failed:|GPU crashed|DXGI_ERROR_DEVICE_REMOVED|DXGI_ERROR_DEVICE_HUNG|EXCEPTION_ACCESS_VIOLATION|SightWeave activation failed|M6P1.*request failed|GRAY_POLICY_LAB_V2_TEN_MIN_FAIL').Count
    perf_pass = $content.Contains('GRAY_POLICY_LAB_V2_TEN_MIN_PASS ')
    pie_stopped = $content.Contains('GRAY_POLICY_LAB_V2_TEN_MIN_PIE_STOPPED')
    callback_unregistered = $content.Contains('GRAY_POLICY_LAB_V2_TEN_MIN_CALLBACK_UNREGISTERED')
...
$reportPath = Join-Path $evidence 'ten_minute_performance.json'
if (Test-Path -LiteralPath $reportPath) {
    $report = Get-Content -Raw -LiteralPath $reportPath | ConvertFrom-Json
    $summary.measured_wall_seconds = $report.wall_seconds
}
$summary.passed = $summary.exit_code -eq 0 -and $summary.severe_lines -eq 0 `
    -and $summary.perf_pass -and $summary.pie_stopped -and $summary.callback_unregistered `
    -and $summary.d3d12_sm6 -and $summary.trace_bytes -gt 0 `
    -and $summary.measured_wall_seconds -ge 600
